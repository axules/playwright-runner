...
exports.resolveQueryPiece = resolveQueryPiece;
 ...
 */
function resolveQueryPiece(locatorPiece) {
...
function resolveQuery(value) {
  const pieces = Array.isArray(value) ? value.flatMap(it => (0, _lodash.default)(it) ? it.split('|>') : it) : value.split('|>');
  return pieces.map(it => (0, _lodash.default)(it) ? resolveQueryPiece(it.trim()) : it);
}
 ...
 */
function resolveLocator(parent, selector) {
  if (!(0, _lodash.default)(selector) && !Array.isArray(selector)) {
    return selector;
  }
  return resolveQuery(selector).reduce((R, [method, ...args]) => R[method](...args), parent);
}
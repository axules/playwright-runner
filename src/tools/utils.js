...
export async function selectElements(container, selector) {
  ...
  return (
    count > 0
      ? (await selectFunc(nodeSelector)).slice(0, count)
      : selectFunc(nodeSelector)
  );
}
...
async function getAttributesInContent(node, selectAttr) {
  const select = (Array.isArray(selectAttr) ? selectAttr : (selectAttr || '').split(','))
    .map(el => el.trim())
    .filter(el => el);
...
export function promiseFlow(list = []) {
  ...
  for (let i = 1; i < list.length; i++) {
    point = point.then(typeof list[i] === 'function' ? list[i] : () => list[i]);
  }
  return point;
}
...
export function matchString(received, expected, strict = false) {
  return expected instanceof RegExp
    ? !!received.match(expected)
    : (strict || !expected ? received == expected : received.includes(expected));
}

export function matchObject(received, expected, strict = false) {
  const diff = Object.entries(expected)
    .map(el => [el[0], undefined, el[1]]);
  const additional = [];
  Object.entries(received).forEach(([k, actual]) => {
    const n = diff.findIndex(el => el && el[0] == k);
    if (n >= 0) {
      if (String(diff[n][2]) == String(actual)) diff[n] = null;
      else diff[n][1] = actual;
    } else {
      additional.push([k, actual, undefined]);
    }
  });

  return diff.concat(strict ? additional : [])
    .filter(el => el);
}
